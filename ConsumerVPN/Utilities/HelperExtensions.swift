...
extension UIAlertController {
	...
	class func presentErrorAlert(_ message: String, in controller: UIViewController) {
...
extension String {
	...
	/// If the returned string is not nil, it is also not empty.
	var whiteSpaceTrimmed: String? {
		let trimmed = trimmingCharacters(in: .whitespaces)
		return trimmed.isEmpty ? nil : trimmed
	}
	
	/// If the returned string is not nil, it appears to be an email
	var emailCharactersOnlyString: String? {
		...
		let trimmedUsername = trimmingCharacters(in: emailCharSet)
		guard trimmedUsername.count <= 0 else {
			return nil
		}
